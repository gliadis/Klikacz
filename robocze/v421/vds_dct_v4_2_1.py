# -*- coding: utf-8 -*-
"""
VBS klikacz NTQ – v4.2.1 (STABLE)

Zmiany v4.2.1:
• FIX: poprawne klikanie kafelka slotu (np. "02:00-02:59 44/130") – w v4.2.0 selektor często nie trafiał.
• Reszta bez zmian: licencja OK/EXPIRED/BLOCKED, Info: wersja/build/zmiany/status licencji, refresh 1-dniowy przez STANDARDOWE.
...
def handle_dialogs(page):
    """Obsługa modalów: Tak / OK / Ok."""
    for txt in ("Tak", "OK", "Ok"):
        try:
...
class Worker(threading.Thread):
                ...
                for day in days:
                    ...
                    for h in ui.iter_hours_for_day(day):
                        ...
                        if used < total:
                            ...
                            self.try_slot(page, slot_key, load_to)
    ...
    def try_slot(self, page, slot_key, load_to):
        """
        FIX v4.2.1:
        kafelek na stronie zwykle ma tekst: "02:00-02:59 44/130"
        więc musimy klikać element, który ZAWIERA slot_key, a nie jest równy slot_key.
        """
        try:
            # Najpierw preferujemy button/a/role=button zawierające slot_key
            candidates = page.locator(
                f"button:has-text('{slot_key}'), a:has-text('{slot_key}'), [role='button']:has-text('{slot_key}')"
            )
            n = candidates.count()

            # Fallback: czasem kafel jest div/span - wtedy łapiemy tekst regexem
            if n == 0:
            ...
            # Potwierdzenia: "Tak" + ewentualne "OK"
            for _ in range(6):
                if not handle_dialogs(page):
                    break
                time.sleep(0.05)
...
class App(tk.Tk):
    ...
    def get_params(self):
        try:
            poll = max(0, int(self.poll_s.get()))
        except Exception:
            poll = 1
        try:
